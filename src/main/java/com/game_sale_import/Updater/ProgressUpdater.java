...
public class ProgressUpdater {
	...
	public static void updateProgress(CsvImportProgress progress, String status, int totalRecords, List<String> errors, CsvImportProgressRepository repository) {
        progress.setStatus(status);
        progress.setEndTime(LocalDateTime.now());

        if ("COMPLETED".equalsIgnoreCase(status)) {
            progress.setTotalRecords(totalRecords);
        } else if ("VALIDATION_ERROR".equalsIgnoreCase(status) && errors != null) {
